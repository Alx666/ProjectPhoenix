		...
		public static void AddScene(IKEffector e, Color color, bool modifiable, float size) {
			...
			if (e.bone != null && weight > 0) {
				...
				if (Application.isPlaying) {
					...
					Handles.SphereHandleCap(0, e.bone.position, Quaternion.identity, size * 0.5f, EventType.Ignore);
					...
					if (rotate) Handles.CubeHandleCap(0, e.position, e.rotation, size, EventType.Ignore);
					else Handles.SphereHandleCap(0, e.position, Quaternion.identity, size, EventType.Ignore);
				}
			}
		}
