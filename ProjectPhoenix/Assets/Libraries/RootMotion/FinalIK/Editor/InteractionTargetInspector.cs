...
namespace RootMotion.FinalIK.Demos {
	...
	public class InteractionTargetInspector : Editor {
		...
		void OnSceneGUI() {
			...
			Handles.SphereHandleCap(0, script.transform.position, Quaternion.identity, size, EventType.Ignore);
			...
			if (script.pivot != null) {
				...
				Handles.SphereHandleCap(0, script.pivot.position, Quaternion.identity, size, EventType.Ignore);
				...
				Handles.SphereHandleCap(0, script.pivot.position + twistAxisWorld, Quaternion.identity, size, EventType.Ignore);

				Handles.CircleHandleCap(0, script.pivot.position, Quaternion.LookRotation(twistAxisWorld), size * 20, EventType.Ignore);
		...
		private void DrawChildrenRecursive(Transform t) {
			for (int i = 0; i < t.childCount; i++) {
				...
				Handles.SphereHandleCap(0, t.GetChild(i).position, Quaternion.identity, size, EventType.Ignore);
