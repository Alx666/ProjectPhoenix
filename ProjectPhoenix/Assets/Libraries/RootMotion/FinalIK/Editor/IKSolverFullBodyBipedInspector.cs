	...
	public class IKSolverFullBodyBipedInspector : IKSolverInspector {
		...
		public static void AddScene(IKSolverFullBodyBiped solver, Color color, bool modifiable, ref int selectedEffector, Transform root) {
			...
			for (int i = 0; i < solver.chain.Length; i++) {
				if (solver.chain[i].nodes.Length == 3 && solver.chain[i].bendConstraint.bendGoal != null && solver.chain[i].bendConstraint.weight > 0f) {
					...
					Handles.SphereHandleCap(0, solver.chain[i].nodes[1].transform.position, Quaternion.identity, size * 0.5f, EventType.Ignore);
					Handles.SphereHandleCap(0, solver.chain[i].bendConstraint.bendGoal.position, Quaternion.identity, size * 0.5f, EventType.Ignore);
		...
		private static void AddLimbHelper(FBIKChain chain, float size, Transform root = null) {
			...
			if (bendDirection != Vector3.zero) {
				...
				Handles.ArrowHandleCap(0, chain.nodes[1].transform.position, Quaternion.LookRotation(bendDirection), size * 2f, EventType.Ignore);
