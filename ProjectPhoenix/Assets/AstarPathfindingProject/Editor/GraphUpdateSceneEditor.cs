	...
	public void OnSceneGUI () {
		...
		for (int i = 0; i < points.Count; i++) {
			if (i == selectedPoint && Tools.current == Tool.Move) {
				...
				Handles.SphereHandleCap(-i-1, points[i], Quaternion.identity, HandleUtility.GetHandleSize(points[i])*pointGizmosRadius*2, EventType.Ignore);
				...
				Handles.SphereHandleCap(-i-1, points[i], Quaternion.identity, HandleUtility.GetHandleSize(points[i])*pointGizmosRadius, EventType.Ignore);
			}
		}
